...
test.skip('handles the flow', async ({ page }) => {
  await page.goto('/en');

  const newsletterForm = await page.getByTestId('newsletter-form');
  await expect(newsletterForm).toBeVisible();

  const personalisationModal = await page.getByTestId('personalisation-modal');
  const emailField = await newsletterForm.getByTestId('email');
  const emailFieldError = await newsletterForm.getByTestId('email.error');
  const submit = await newsletterForm.getByTestId('newsletter-form.submit');
  ...
  const introSuccess = await page.getByTestId('personalisation-modal.intro-newsletter');
  await expect(introSuccess).toBeVisible();

  const title = await introSuccess.getByTestId('personalisation-modal.intro-newsletter.title');
  await expect(title).toContainText('john.doe@example.com');
...
test.skip('prefills a user data once logged in', async ({ page }) => {
  ...
  const personalisationNameField = await personalisationForm.locator('input[name="name"]');

  // Check if email is passed correctly
  expect(await personalisationEmailField.inputValue()).toEqual('john.doe@example.com');
  expect(await personalisationNameField.inputValue()).toEqual('John');
});
